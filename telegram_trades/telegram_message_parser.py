...
def get_all_contract_details(exchange=None):
    ...
    """
    dfs = []
    req_columns = ['Exch', 'Symbol', 'Option Type', 'Strike Price', 'Trading Symbol','Expiry Date']
...
def write_signals_to_csv(signal_details):
    with open(signals_csv_filename, 'a', newline='') as file:
        ...
        writer.writerow({k: signal_details.get(k, '') for k in signals_csv_file_headers})
...
def write_failure_to_csv(failure_details):
    with open(failure_csv_filename, 'a', newline='') as file:
        ...
        writer.writerow({k: failure_details.get(k, '') for k in failure_csv_file_headers})
...
class PaidCallPut():
    ...
    def coin_option_name(self, df, symbol, date, month, strike, option_type):
        filtered_df = df[(df["Exch"]=="NFO") & (df["Symbol"]==symbol) & (df["Strike Price"]==float(strike)) & (df["Option Type"]==option_type) & (df['Expiry Date']==f'2024-{month}-{date}')]
    ...
    def get_signal(self):
        try:
            ...
            for i, word in enumerate(req_content):
                if word.upper().strip()=="BUY" and i+2 <= len(req_content)+1 and strike == None:
            ...
            symbol_dict = self.coin_option_name(scrip_info_df, symbol, date, month, strike, option)
            symbol = symbol_dict["Trading Symbol"]
